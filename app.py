...
# --- Email (optional — only active if these are all set on the server) -----------------------
...
# --- PDF report: download or email ------------------------------------------------------------

def _load_photo_and_report(photo_name, report_filename):
    photo_path = Path("input") / secure_filename(photo_name or "")
    report_path = Path("outputs") / secure_filename(report_filename or "")
    if not photo_path.exists() or not report_path.exists():
        abort(404)
    return photo_path, report_path.read_text()


def _pdf_filename(photo_name: str) -> str:
    return f"{Path(secure_filename(photo_name)).stem}_QAQC_report.pdf"


@app.route("/evaluate/pdf")
def evaluate_pdf():
    photo_name = request.args.get("photo")
    report_filename = request.args.get("report")
    photo_path, report_text = _load_photo_and_report(photo_name, report_filename)
    pdf_bytes = pdf_report.build_report_pdf(photo_path, photo_name, report_text)
    return send_file(io.BytesIO(pdf_bytes), mimetype="application/pdf",
                      as_attachment=True, download_name=_pdf_filename(photo_name))


@app.route("/evaluate/email", methods=["GET", "POST"])
def evaluate_email():
    photo_name = request.args.get("photo") or request.form.get("photo")
    report_filename = request.args.get("report") or request.form.get("report")

    if request.method == "GET":
        return render_template("evaluate_email.html", photo_name=photo_name,
                                report_filename=report_filename, email_configured=EMAIL_CONFIGURED)

    if not EMAIL_CONFIGURED:
        flash("Email isn't set up on the server yet (SMTP_HOST/SMTP_USER/SMTP_PASSWORD are "
              "missing). Download the PDF instead, or ask whoever manages Render to add those.", "error")
        return redirect(url_for("evaluate_email", photo=photo_name, report=report_filename))
    ...
    try:
        ...
        msg = EmailMessage()
        msg["Subject"] = f"QA/QC Report: {photo_name}"
        msg["From"] = SMTP_FROM
        msg["To"] = recipient
        msg.set_content(f"Attached: QA/QC evaluation report for {photo_name}.")
        msg.add_attachment(pdf_bytes, maintype="application", subtype="pdf",
                            filename=_pdf_filename(photo_name))

        with smtplib.SMTP(SMTP_HOST, SMTP_PORT) as server:
            server.starttls()
            server.login(SMTP_USER, SMTP_PASSWORD)
            server.send_message(msg)
        flash(f"Emailed report to {recipient}.", "success")
    except Exception as e:
        app.logger.exception("Failed to send report email")
        flash(f"Couldn't send email: {type(e).__name__}: {e}", "error")
