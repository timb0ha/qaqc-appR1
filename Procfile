...
## Issues Found
One entry per issue marked ISSUE FOUND: name the issue and describe what's wrong in 1-2 concise \
sentences (multiple incidents noted briefly if applicable), citing the relevant training \
example, stored feedback, submittal, or web-sourced standard only when it materially supports \
the finding. This report is formatted as a one-page PDF, so keep entries tight by default — \
expand beyond 1-2 sentences only for a Critical/Major defect that genuinely needs more \
explanation to be actionable in the field, or when there are numerous high-priority defects that \
warrant fuller documentation. Also include here, clearly prefixed "Additional (not on \
checklist):", any further problems you identify from broader review / web search that aren't on \
the given checklist. If nothing belongs in this section, write "None."
